...
### 1. `ExportTask.chat_id = 0` для username-based экспорта
- **Файл:** `tg_exporter_cli/commands/export.py:214-222`
...
### 2. `auth verify` расхождение exit-кодов с docstring
- **Файл:** `tg_exporter_cli/commands/auth.py:128-137`
- **Описание:** Docstring обещает код 2 (нет сессии), реально всегда возвращается 1.

### 3. DI-контейнер пересобирается при каждом вызове подкоманды
- **Файл:** `tg_exporter_cli/main.py:19-22`
- **Описание:** `CliHost().build()` выполняется при каждом запуске, включая `--help`.

### 4. Сборочные скрипты не устанавливают системные зависимости
- **Файл:** `scripts/build_mac.sh`
- **Описание:** Отсутствуют `brew install` для зависимостей PyInstaller/faster-whisper.

### 5. Имя файла не соответствует имени класса: `telegram_client_manager.py` → `TelethonClientManager`
...
### 6. Имя файла не соответствует имени класса: `logger.py` → `AppLogger`
- **Файл:** `tg_exporter/utils/logger.py:47`
- **Описание:** Нарушение правила snake_case от имени класса.

### 7. Несогласованность ABC-интерфейсов
...
### 8. `keyring_secret_store.py` использует стандартный `logging` вместо `AppLogger`
...
### 9. Секреты в `StaticConfig` (DI-singleton)
...
### 10. `_extract_links` использует `type().__name__` вместо `isinstance()`
