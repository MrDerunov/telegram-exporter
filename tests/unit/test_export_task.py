...
class TestExportTask:
    ...
    def test_match_all_when_author_filter_is_empty(self):
        af = AuthorFilter()
        assert af.matches(123)
        assert af.matches(None)
        assert af.is_empty()

    def test_match_only_specified_ids(self):
    ...
    def test_set_status_to_cancelled_on_cancel(self):
        p = ExportProgress()
        p.start()
        p.cancel()
        assert p.status == ExportStatus.CANCELLED

    def test_set_status_to_error_on_fail(self):
        p = ExportProgress()
        p.start()
    ...
    def test_cap_progress_ratio_at_1(self):
        p = ExportProgress()
        p.total_messages = 10
